...
dkb:
	docker build -t consumer-report-user .

dkr:
	docker run -p "8280:8280" consumer-report-user

launch: dkb dkr

cr-log:
	docker logs consumer-report-user -f
...
cr-ssh:
	docker exec -it consumer-report-user /bin/bash
